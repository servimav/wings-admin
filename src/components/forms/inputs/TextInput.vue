...
<template>
	<div>
		<label :for="id" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white" v-if="label">{{ label }}</label>

		<textarea v-if="type === 'textarea'" @change="onChange" :value="modelValue" :name="id" :id="id"
		...
		<input v-else @change="onChange" :value="modelValue" :type="realType" :name="id" :id="id" :placeholder="placeholder"
			class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
