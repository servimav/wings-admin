...
<template>
	...
	<div class="fixed bottom-6 left-6 z-10">
		<FloatButton v-model="showFloatMenu">
			<FloatButtonItem @click="() => {
					showFloatMenu = false
					showNewOfferModal = true
				}
				" label="Nuevo" :icon="IconPlus" id="plus-float" />
			<FloatButtonItem @click="() => {
					showFloatMenu = false
					showFilterModal = true
				}
				" label="Buscar" :icon="IconSearch" id="search-float" />
		</FloatButton>
	</div>
	...
	<BaseModal v-if="showFilterModal" @close="() => (showFilterModal = false)">
		<div class="flex items-center justify-center">
			<div class="z-30 w-full rounded-xl border bg-white p-4">
				<SearchForm :filter="filter" @close="() => (showFilterModal = false)" @search="onSearchOffer" />
			</div>
		</div>
	</BaseModal>
	<!-- / Search Modal -->
